...
def receiveOnePing(mySocket, ID, sequence, destAddr, timeout):
    ...
    while 1:
        ...
        temp = time.time()#实际上利用select就是让socket先完成数据接收，因此这里temp和timeRec是一样的
        #或者不利用select，那么则需要将rec的计时放在recvfrom的下一行，否则会导致delay为0
        ...
        if type == 0 and packetID == ID:  # type should be 0
            ...
            timeSent = struct.unpack("!d", recPacket[28: 28 + byte_in_double])[0] #最后几位为什么解包可以推出时间？
            #nmb，发送的是带时间戳的，最后几位就是发送时间
